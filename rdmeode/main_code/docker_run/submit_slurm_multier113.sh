...
do	
	sbatch --output ./logfile1.13_ermulti_$(date +%Y-%m-%d)_$start_idx.$exgal.$time.log --job "yeermt13_"$start_idx --gres=gpu:$gpus submit_job_multier113.sh $start_idx $exgal $time $gpus $tags $geo $fix
	((start_idx++))
done
