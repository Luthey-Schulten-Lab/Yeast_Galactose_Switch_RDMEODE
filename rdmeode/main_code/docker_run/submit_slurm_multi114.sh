...
do	
	sbatch --output ./logfile1.14_multi_$(date +%Y-%m-%d)_$start_idx.$exgal.$time.log \
	       --job-name="yemt14_"$start_idx \
	       --gres=gpu:$gpus \
	       --mail-type=BEGIN,FAIL,END \
	      
	       submit_job_multi114.sh $start_idx $exgal $time $gpus $tags $geo $fix
	((start_idx++))
done
