...
NV_GPU=$CUDA_VISIBLE_DEVICES nvidia-docker run --name "yeastmt14_"$i --rm -v $(pwd):/workspace lm2.5_dev:1005v2  /workspace/galactose_rdmeode1.14_multi_noreg.py -id $i -t $time -g $exgal -gpus $gpus -tag $tag -geo $geo -geloc $fixgene
