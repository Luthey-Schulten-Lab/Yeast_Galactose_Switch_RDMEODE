...
echo "Executing Python command: python /workspace/galactose_rdmeode1.14_multi.py -id \"$id\" -t \"$t\" -g \"$g\" -gpus \"$gpus\" -tag \"$tag\" -geo \"$geo\" -mt \"$maxtime\" -geloc \"$fix_gene\" -ckpt \"$ckpt\""

# Run your Python script with the input parameters
python /workspace/galactose_rdmeode1.14_multi.py   -id "$id" -t "$t" -g "$g" -gpus "$gpus" -tag "$tag" -geo "$geo" -mt "$maxtime" -geloc "$fix_gene" -ckpt "$ckpt"
