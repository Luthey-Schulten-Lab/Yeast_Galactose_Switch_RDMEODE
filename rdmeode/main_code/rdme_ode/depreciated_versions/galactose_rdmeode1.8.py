#!/usr/bin/env python
# coding: utf-8
'''build by Tianyu Wu v 1.8  : 
1.  fix critical bugs in hook simulation
2. support IF_DGX for cython import 
'''
...
# In[1]:
import argparse
parser = argparse.ArgumentParser(description='Accept three parameters: output index, simtime(sec)  and external galactose concentration(mM)')
parser.add_argument('-id', '--index',  type=int, required=True, help='index of the output lm files')
parser.add_argument('-t', '--simtime',  type=float, default=60*60, help='simulation time')
...
sim.simulationTime = simtime  # seconds
