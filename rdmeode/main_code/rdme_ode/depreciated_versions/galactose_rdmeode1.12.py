...
1.12.2 change the timestep back to default instead of 67e-6
1.12.3 add the tag for the output folder
1.12.4 don't use the multi-gpu simulation for now, depreciated
1.12.5 0902 solve the concentration issue for copy and init ys, compatible with kinetic paramters
1.12.6 add a timer for total simulation time 
merge from 1.10
1.10.9 add diffusion rate to the ribosomes as 0 inside the ribosomes, to prevent ribosomes diffuse inside the ribosomes
...
if gpus != 1:
    raise ValueError("Error: Only single GPU simulation is supported at this moment.")

import datetime
date = datetime.datetime.now().strftime("%Y%m%d")
if IF_DGX == True:
    if gpus == 1:
        output_folder = "workspace/yeast_1.12_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + tag_output +".lm"
    else:
        output_folder = "workspace/yeast_multi_1.12_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + tag_output +"_gpu"+ str(gpus) +".lm"
else:
    if gpus == 1:
        output_folder = "yeast_1.12_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + tag_output +".lm"
    else:
        output_folder = "yeast_multi_1.12_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + tag_output +"_gpu"+ str(gpus) +".lm"
...
# region is actually applied to the lattice by RegionBuilder function 
B = RegionBuilder(sim)
...
with sim.construct():
    ...
    for mrna in sim.speciesList.matchRegex("R.*"):
        ...
        # ribosome occlusion
        sim.transitionRate(mrna, reg.ribosomes, reg.ribosomes, sim.diffusionZero)
...
with sim.construct():
    ...
    for sps in [sp.G1, sp.G2, sp.G3, sp.G3i, sp.G4, sp.G4d, sp.G80, sp.G80d, sp.G80d_G3i, sp.Grep]:
        ...
        # ribosome occlusion
        sim.transitionRate(sps, reg.ribosomes, reg.ribosomes, sim.diffusionZero)
...
class OdeRdmeHybridSolver:
    ...
    communication step.
    initialExternalGalactose is in M
    """
    def __init__(self, lmFile, initialExternalGalactose):
        ...
        # ode reaction parameters
        self.odeYs = None  # the current state of the ODE system, unit in concentration(M)
        self.lastOdeEval = 0  # the last time the ODE system was evaluated
        self.odeEvals = []  # a list of tuples, each tuple contains the time and the state of the ODE system
    ...
    def rdme2odeConc(self, y0, cts):
        '''
        input:
        y0: the current state of the ODE system unit in concentration
        cts: the current state of the RDME system unit in counts
        output:
        y: the updated state of the ODE system unit in concentration
        '''
        y = y0.copy()
        # update G1 in ODE
    ...
    def hookSimulation(self, t, lattice):
        ...
        '''
        # start time tracker
        start_time = time.time()  # Start timer for this hook call
        ...
        if self.odeYs is None:
            # concentration(M)
            print("concentration intialized")
            ys0 = self.copyInitialConditions(cts)
        else:
            # concentration(M)
            print("concentration updated from RDME")
            ys0 = self.rdme2odeConc(self.odeYs, cts)
                   
        dt = t-self.lastOdeEval
        if dt>0:
            # perform ode simulation here
            ys1 = self.stepOde(dt, ys0)
        else:
            #  if next step is smaller than the last step, keep the last step value 
            ys1 = ys0
        # add the current step to the list for saving purpose 
        # unit in concentration(M), time is seconds
        ...
        # end time tracker
        end_time = time.time()
        self.hook_time += end_time - start_time  # Add time spent in this hook call
...
if traj is not None:
    ...
    with open(traj.filename + "_ode.pkl", "wb") as f:
        pickle.dump(odeData, f)
...
print(f"Total time spent in simulation: {total_time} seconds")
print(f"Total time taken: {int(days)} days, {int(hours)} hours, {int(minutes)} minutes")
