#!/usr/bin/env python
# coding: utf-8
'''build by Tianyu Wu v 1.7  : 
1.  change dt to 67e-6s
2. change the diffusion rate of mRNA to 0.05e-12 m^2/s
3. remove dummy rna
'''
...
parser.add_argument('-t', '--simtime',  type=int, default=60*60, help='simulation time')
...
if IF_DGX == True:
    output_folder = "workspace/yeast_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" +".lm"
else: 
    output_folder = "yeast_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" +".lm"
...
mRNADiffusion = 0.05e-12 # 2018, IET
...
with sim.construct():
    ...
    sim.reaction([sp.G4d], [sp.G4, sp.G80], rc.rd, annotation ="Gal4p/Gal80p dimer dissociation", regions=[reg.cytoplasm, reg.nucleoplasm])
    sim.reaction([sp.G80, sp.G4d], [sp.G80d], rc.fd, annotation ="Gal80p/Gal80p dimer formation", regions=[reg.cytoplasm, reg.nucleoplasm])
...
with sim.construct():
    ...
    for dna, dna_gal4, dna_gal4_gal80, f_g4, r_g4, f_g4g80, r_g4g80 in zip(dnas, dna_gal4, dna_gal4_gal80, f_g4s, r_g4s, f_g4g80s, r_g4g80s):
        sim.reaction([dna, sp.G4], [dna_gal4], f_g4, annotation="Gal4p binding to gene", regions=reg.nucleoplasm)
        sim.reaction([dna_gal4], [dna, sp.G4], r_g4, annotation="Gal4p dissociation from gene", regions=reg.nucleoplasm)
        sim.reaction([dna_gal4, sp.G80], [dna_gal4_gal80], f_g4g80, annotation="Gal80p binding to gene", regions=reg.nucleoplasm)
        sim.reaction([dna_gal4_gal80], [dna_gal4, sp.G80], r_g4g80, annotation="Gal80p dissociation from gene", regions=reg.nucleoplasm)
...
with sim.construct():
    ...
    ## Gal3p* Gal80p
    sim.reaction([sp.G3i, sp.G80], [sp.G80d_G3i], rc.fd3i80, annotation="Gal3p*/Gal80 association", regions=reg.cytoplasm)
    sim.reaction([sp.G80d_G3i], [sp.G3i, sp.G80], rc.dr3i80, annotation="Gal3p*/Gal80 disassociation", regions=reg.cytoplasm)
...
class OdeRdmeHybridSolver:
    '''Transport reactions and galactose metabolism handled by ODE, rest by RDME. 
    '''
    def __init__(self, lmFile, initialExternalGalactose):
        ...
        self.g3actRc = self.rdme.rc.fi._toLM() # this is reaction rate constant for Gal3 activation
        
        
    def copyInitialConditions(self, cts):
        # copy initial counts from RDME
        y = np.zeros(len(self.odeSpNames))
        y[self.odeSpIndex("GAI")] = 0
        #XXX need to change from countBySpecies to counts bySpeciesRegion 
        y[self.odeSpIndex("G1")] = cts['countBySpecies'][self.rdme.sp.G1]/self.NAV # XXX think about which volume to use, clearly here is all volume
        y[self.odeSpIndex("G1GAI")] = 0
        y[self.odeSpIndex("G2")] = cts['countBySpecies'][self.rdme.sp.G2]/self.NAV # XXX think about which volume to use
        y[self.odeSpIndex("G2GAE")] = 0
        y[self.odeSpIndex("G2GAI")] = 0
        # GAI(internal galactose) in complexes state G3i, G80d_G3i
        self.boundGal = self.rdmeGal(cts)
        return y
    
    def rdmeGal(self, cts):
        # internal galactose is in G3i and G80d_G3i
        return cts['countBySpecies'][self.rdme.sp.G3i] + cts['countBySpecies'][self.rdme.sp.G80d_G3i]
    
    def odeSpIndex(self, sp):
        return self.odeSpNames.index(sp)
    
    def rdme2odeConentration(self, y0, cts):
        # update the concentration in DOE from RDME
        y = y0.copy()
        # update Gal1 
        ...
        change_g1 = g1rdme - g1ode - g1gaiode
        if change_g1 > 0:
            y[self.odeSpIndex("G1")] = g1ode + change_g1
        else:
    ...
    def buildOdeSys(self, rxns):
        # rxns in format [[reactants], [products], rate], ...]
        odeSpNames = set()
        for react, prod, _ in rxns:
            odeSpNames |= set(prod) | set(react)
        nsp = len(odeSpNames) # number of species
        nrxn = len(rxns)    # number of reactions
        stoch = np.zeros((nrxn, nsp), dtype=np.int32)   #Stoichiometric Matrix
        dep = np.zeros((nrxn, nsp), dtype=np.int32)     #Dependency Matrix, species counts consumed
    ...
    def stepOde(self, t, ys0):
        ...
        #linear distribution and solve the function 
        ts = np.linspace(0,t, 10)
        ys = spint.odeint(rhs.evaluate, ys0, ts)
        return ys[-1,:] # only return the final results
    def print_status(self, t, cts, assocRt):
        ...
        # Print the ODE section header
        print("ODE Results:")
        
        # Print the species names and their respective values from the ODE
        for i, n in enumerate(self.odeSpNames):
            print(f"  {n:<16}{self.odeYs[i]:16.5g}")
        
        # Print the RDME section header
        print("RDME Results:")
        
        # Print the species names and their respective counts from the RDME
        for n in self.rdme.speciesList:
            print(f"  {n.name:<16}{cts['countBySpecies'][n]:16d}")
        
        # Print the new rate
        print(f"New rate: {assocRt:.3g}")
    ...
    def hookSimulation(self, t, lattice):
        ...
        #show the status
        self.print_status(t, cts, assocRt)
