...
in v 1.13
1.13.1 split the region obstacle into corresponding regions
in v1.14 
...
1.14.5 support gene location to be center or edge
in v 1.15
1.15.1 add chromosome region and gene location 
'''
# write a timer 
import time
import signal
...
parser.add_argument('-geloc', '--gene_location', type=str, default='random', help='location of the genes, default is random, also can be center or edge or chromosome')
...
# If checkpoint file is provided, extract the directory from it
if checkpoint_file:
    # Extract the directory from the checkpoint file path
    output_dir = os.path.dirname(checkpoint_file)
    print(f"Using directory from checkpoint file: {output_dir}")
else:
    # Create a new directory based on the output_order
    output_dir = "simulation_results_id_" + str(output_order)

if IF_DGX == True:
    
    output_folder = "workspace/simulation_results_id_" + str(output_order) + "/yeast" + version + "_multi_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + output_tag +"_gpu"+ str(gpus) +".lm"
...
chromo_gene = np.load("gene_masks.npy").astype(bool)
chromo_dummy = np.load("dummy_chromosome.npy").astype(bool)
# print("chromo_gene shape: ", chromo_gene.shape)
# print("chromo_dummy shape: ", chromo_dummy.shape)
# print(f"unique values in chromo_gene: {np.unique(chromo_gene)}")
# print(f"unique values in chromo_dummy: {np.unique(chromo_dummy)}")
# print(f"unique values in membrane: {np.unique(membrane)}")
# breakpoint()
...
if checkpoint_file == "":
    ...
    if gene_location == "random":
        ...
        for i, gene in enumerate(genes):
            if gene == "DG4":
                sp.DG4.placeParticle(*gene_coordiantes['DG4'], 1)
                print("{} in state {} at position {}".format("Gene4", "DG4", gene_coordiantes['DG4']))
            else:
                ops = [gene+x for x in ["", "_G4d", "_G4d_G80d"]]
                spName = max(ops, key=lambda x:initMolec[x])
                sim.species(spName).placeParticle(*gene_coordiantes[gene], 1)
                print("{} in state {} at position {}".format(gene, spName, gene_coordiantes[gene]))
    elif gene_location == "center":
        print("gene location center")
        # Find the center of the nucleoplasm
        ...
        # Ensure the center point is actually in the nucleoplasm
        if sim.siteLattice[tuple(center)] != reg.nucleoplasm.idx:
            # Find the closest point in the nucleoplasm
        ...
        # Place genes at or near the center
        genes = ["DG1", "DG2", "DG3", "DG80", "DGrep", "DG4"]
        positions = []
        
        # Place first gene at center
        x, y, z = center
        positions.append((x, y, z))
        
        # Place other genes in a small cluster around the center
        for i in range(1, len(genes)):
            ...
            # If no position found, find another valid position in nucleoplasm
            if len(positions) <= i:
        ...
        # Place genes at the determined positions
        for i, gene in enumerate(genes):
            if gene == "DG4":
        ...
        # Get the nucleoplasm and nuclear pores indices
        nucleoplasm_only_indices = np.argwhere(boolLattice("nucleoplasm"))
        nuclear_pores_indices = np.argwhere(boolLattice("nuclearPores"))
        
        # Create a set of nucleoplasm voxel coordinates for faster lookup
        nucleoplasm_set = {tuple(idx) for idx in nucleoplasm_only_indices}
        
        # Create a set of nuclear pores voxel coordinates to exclude
        nuclear_pores_set = {tuple(idx) for idx in nuclear_pores_indices}
        
        # Find edge voxels (nucleoplasm voxels that have at least one non-nucleoplasm neighbor)
        edge_voxels = []
        ...
        for idx in nucleoplasm_only_indices:
            ...
            # Check if this voxel has at least one non-nucleoplasm neighbor
            for dx, dy, dz in neighbor_offsets:
                nx, ny, nz = x + dx, y + dy, z + dz
                
                # Check if the neighbor is outside the lattice bounds
                ...
                # If the neighbor is not in nucleoplasm, this is an edge voxel
        ...
        # If we don't have enough edge voxels, use some interior voxels
        if len(edge_voxels) < 6:
            print("Warning: Not enough edge voxels found, using some interior voxels")
            # Get interior voxels (not on edge and not nuclear pores)
            ...
            # Add some interior voxels to edge_voxels
        ...
        # If we couldn't place all genes, place the remaining ones randomly in nucleoplasm
...
with sim.construct():
    ...
    # THis is for mRNA to leave the chromosome region
    for mrna in sim.speciesList.matchRegex("R.*"):
...
# 
# allow them to diffuse into the nucleoplasm, and diffuse in the chromosome
