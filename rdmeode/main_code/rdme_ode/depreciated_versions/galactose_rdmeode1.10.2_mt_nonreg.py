...
This is the version for non-regulation case which aims to reproduce the result in the paper by Ramsey et al. 2024

In short, they replace the promoter for Gal80 and Gal3, so that the Gal80 and Gal3 are always expressed constitutively
the new promoter is called CYC1
'''
# write a timer 
import time
import signal
...
if IF_DGX == True:
    if gpus == 1:
        ...
        output_folder = "workspace/yeast" +version +"_multi" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + output_tag +"_gpu"+ str(gpus) +".lm"
else:
    if gpus == 1:
        output_folder = "yeast" +version +"_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + output_tag +".lm"
    else:
        output_folder = "yeast" +version +"_multi" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + output_tag +"_gpu"+ str(gpus) +".lm"
...
sim = RDMESim("Galactose switch, RDME/ODE hybrid no regulation",
              output_folder,
...
with sim.construct():
    ...
    # change the transcription states for G3 and G80 without binding to G4d
    ...
    genes               = [sp.DG1_G4d,  sp.DG2_G4d, sp.DG3,     sp.DG4,     sp.DGrep,       sp.DG80]
