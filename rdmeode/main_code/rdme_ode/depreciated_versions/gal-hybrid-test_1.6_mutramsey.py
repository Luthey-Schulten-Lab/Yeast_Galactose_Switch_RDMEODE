#!/usr/bin/env python
# coding: utf-8
'''build by Tianyu Wu v 1.6  : 
1.  for mutants, genes for Gal3 and Gal80 no longer under regulation.
2. change externalGal to externalGal_input to differentiate wtih default parameter
3. IF_DGX parameter to for quick conversion to use on DGX
'''
...
if IF_DGX == True:
    ...
    latticeData = pickle.load(lzma.open("yeast-lattice.pkl.xz", "rb"))
...
sim.simulationTime= simtime * 60 # sec
sim.timestep = 50e-6
...
with sim.construct():
    ...
    sim.species('DG3_CYC1', texRepr='CYC1:D_{3}', annotation="GAL3 gene bound with CYC1")
...
with sim.construct():
    ...
    sim.species('DG80_CYC1', texRepr='CYC1:D_{80}', annotation="GAL80 gene bound with CYC1")
...
# For mutant yeast, the gene DG3 and DG80 dont need to bind with G4d or G80d, we assume they bind with CYC-1 on the promoting area and can be transcribed constantly.
...
with sim.construct():
    ...
    mrnas =     [sp.R1,           sp.R2,           sp.RGrep]
    dnaActs =   [sp.DG1_G4d,      sp.DG2_G4d,      sp.DGrep_G4d]
    dnaDeacts = [sp.DG1_G4d_G80d, sp.DG2_G4d_G80d, sp.DGrep_G4d_G80d]
    f1s =       [rc.f1_4,         rc.f1_5,         rc.f1_4]
    f2s =       [rc.f2_4,         rc.f2_5,         rc.f2_4]
    r1s =       [rc.r1_4,         rc.r1_5,         rc.r1_4]
    r2s =       [rc.r2_4,         rc.r2_5,         rc.r2_4]
...
# Here, we need to define the transcription of G3 and G80 separately as: 
# 
# $$ 
# CYC1\_DG_3 \longrightarrow CYC1\_DG_3 +RNA_3\\
# 
# CYC1\_DG_{80} \longrightarrow CYC1\_DG_{80} +RNA_{80}
# 
# $$
# 
# transcription rate will be their rate times $f_{mut}$:
# $$
# k_{transG3} = k_{ir,gal3} \times f_{mut]} = 0.7465 \times 0.04 molec\cdot min^{-1}\\
# k_{transG3} = k_{ir,gal3} \times f_{mut]} = 0.6065 \times 0.04 molec\cdot min^{-1}
# $$
...
with sim.construct():
    f_mut = 0.04 # reference from 0.04 -0.06
    ...
    sim.rateConst("alpha3", 0.7465*f_mut*conv1stOrder, order=1, annotation='GAL3 transcription') # times mutant factor 0.04-0.06 from Ramsey, 2006
    ...
    sim.rateConst("alpha80", 0.6065*f_mut*conv1stOrder, order=1, annotation='GAL80 transcription') # times mutant factor 0.04-0.06 from Ramsey, 2006
    ...
    tss =   [rc.alpha1,   rc.alpha2,   rc.alpha3,   rc.ir_gal4,  rc.alpha_rep, rc.alpha80]      # transcription rates
    dcys =  [rc.dr_gal1,  rc.dr_gal2,  rc.dr_gal3,  rc.dr_gal4,  rc.dr_rep,    rc.dr_gal80]     # degradation rates
    genes = [sp.DG1_G4d,  sp.DG2_G4d,  sp.DG3_CYC1,  sp.DG4,      sp.DGrep_G4d, sp.DG80_CYC1]
...
# change back to original way to load the init counts
...
if True:
    ...
    sp.DG3_CYC1.placeNumberInto(reg.nucleoplasm, 1)
    sp.DG80_CYC1.placeNumberInto(reg.nucleoplasm, 1)
    ...
    fname = "/home/tianyu/Programs/yeast_hybrid_2018/yeast_hybrid_CME-ODE/ode_rdme/icLattice.lm"
