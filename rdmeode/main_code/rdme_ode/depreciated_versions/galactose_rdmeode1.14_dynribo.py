...
# Number of cytoplasmic ribosomes
# nRibosomes = np.sum(ribosomes)

# diffusion coefficients for mRNA
# mRNADiffusion = 0.5e-12 # m^2/s
mRNADiffusion = 0.05e-12 # number from sif 
#D t = 8.42 × 10 − 11 m2/s and D r = 3 × 10 − 13 m2/s (Politz et al., 2003; Werner, 2011)
ribo_diffusion = 3e-13 # number from sif
...
with sim.construct():
    ...
    for trans_rate, decay_rate, gene, mrna in zip(transcription_rates, decay_rates, genes, mrnas):
        ...
        sim.reaction([mrna], [], decay_rate, regions=[reg.nucleoplasm, reg.cytoplasm])
...
with sim.construct():
    ...
    for mrna, translatingRibosomes, protein, ktl, dcy, mdcy in zip(mrnas, translatingRibosomes, prots, ktls, dcys, mdcys):
        #association
        sim.reaction([sp.ribosome, mrna], [translatingRibosomes], rc.rib_assoc, regions=reg.cytoplasm)
        #translation
        sim.reaction([translatingRibosomes], [sp.ribosome, mrna, protein], ktl, regions=reg.cytoplasm)
        #degradation in association form
        sim.reaction([translatingRibosomes], [sp.ribosome], mdcy, regions=reg.cytoplasm)
...
with sim.construct():
    deg_compartments =         [[reg.cytoplasm], 
                                [reg.cytoplasm, reg.plasmaMembrane], 
                                [reg.cytoplasm], 
                                [reg.cytoplasm, reg.nucleoplasm], 
                                [reg.cytoplasm], 
                                [reg.cytoplasm, reg.nucleoplasm]]
...
if checkpoint_file == "":
    ...
    for x, y, z in np.argwhere(ribosomes):
        sp.ribosome.placeParticle(x, y, z, 1)

    print("ribosomes number:", np.sum(ribosomes))
else:
...
# ## Ribosomes
# 
# move in the cytoplasm as particles
# In[63]:


with sim.construct():
    for sps in sim.speciesList.matchRegex("ribosome.*"):
        sim.transitionRate(sps, reg.cytoplasm, reg.cytoplasm, ribo_diffusion)
