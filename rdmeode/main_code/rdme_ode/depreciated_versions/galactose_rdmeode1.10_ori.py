...
1.10.4 remove decimation 
1.10.5 add tag for the output folder
1.10.6 change the timestep to 50e-6
1.10.7 add timer for the simulation
1.10.8 add save the region function
...
if IF_DGX == True:
    output_folder = "workspace/yeast_1.10_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + output_tag +".lm"
else: 
    output_folder = "yeast_1.10_" + str(date) + "_" + str(output_order) + "_t"+ str(simtime) + "min"+ "GAE" + str(externalGal_input)+ "mM" + output_tag +".lm"
...
with sim.construct():
    ...
    for mrna in sim.speciesList.matchRegex("R.*"):
        ...
        #add the rule to prevent diffuse inside ribosomes
...
with sim.construct():
    ...
    for sps in [sp.G1, sp.G2, sp.G3, sp.G3i, sp.G4, sp.G4d, sp.G80, sp.G80d, sp.G80d_G3i, sp.Grep]:
        ...
        # add several rules to prevent diffuse inside ribosomes
...
class OdeRdmeHybridSolver:
    ...
    def __init__(self, lmFile, initialExternalGalactose):
        ...
        self.save_cts_by_region_file = f"test_species_counts_by_region.jsonl"
    ...
    def hookSimulation(self, t, lattice):
        ...
        self.save_rdme_cts_by_region(t)
        # print(self.rdme.reactionList[self.g3actRidx].rate.value)
...
#close the output file for region cts
...
print(f"Total time taken in seconds: {int(total_time)}")
print(f"Total time taken: {int(days)} days, {int(hours)} hours, {int(minutes)} minutes, {int(seconds)} seconds")