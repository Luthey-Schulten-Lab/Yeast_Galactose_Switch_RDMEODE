#!/usr/bin/env python
# coding: utf-8
'''Combined simulation file with optimized ribosome movement
Version: 1.17_ribosome_movement
Features:
...
- Optimized ribosome movement for 300k+ ribosomes
- Two movement modes: diffusion-based (fast) or hook-based (controlled)
'''
...
from lm_functions import deleteParticle, checkParticle, getParticlesInSite
IF_DGX = False
version = "1.17_ribosome_movement"
...
# Ribosome movement arguments
parser.add_argument('--enable-ribosome-movement', action='store_true', default=False, help='Enable ribosome movement')
parser.add_argument('--ribosome-movement-mode', type=str, default='hook', choices=['diffusion', 'hook'], 
                    help='Movement mode: diffusion (fast, solver-handled) or hook (controlled, slower)')
parser.add_argument('--ribosome-diffusion-rate', type=float, default=1e-14, help='Diffusion rate for ribosomes (m^2/s) when using diffusion mode')
parser.add_argument('--ribosome-move-fraction', type=float, default=1.0, help='Fraction of ribosomes to move per hook call (0.0-1.0) when using hook mode')
parser.add_argument('--ribosome-move-interval', type=int, default=1, help='Move ribosomes every N hook calls (reduces overhead, default 1)')
...
ribosome_diffusion_rate = args.ribosome_diffusion_rate
ribosome_move_fraction = args.ribosome_move_fraction
ribosome_move_interval = args.ribosome_move_interval
print(f"GPUs: {gpus}")
if enable_ribosome_movement:
    print(f"Ribosome movement: ENABLED (mode: {ribosome_movement_mode})")
    if ribosome_movement_mode == 'diffusion':
        print(f"  Diffusion rate: {ribosome_diffusion_rate} m^2/s")
    else:
        print(f"  Move fraction: {ribosome_move_fraction}, Interval: every {ribosome_move_interval} hooks")
...
if enable_ribosome_movement:
    feature_suffix += f"_RIBO_MOVE_{ribosome_movement_mode}"
if IF_DGX:
    dir_dgx = "workspace/"
else:
    dir_dgx = ""
...
class OdeRdmeHybridSolver:
    ...
    def _cache_valid_ribo_positions(self, site_lattice):
        ...
        # Build a boolean mask for valid ribosome positions
        valid_positions = np.zeros(site_lattice.shape, dtype=bool)
    ...
    def _move_ribosomes_hook(self, lattice):
        ...
        try:
            particles = lattice.getParticleLattice()
        except AttributeError:
            particles = None
            print("Warning: Could not get raw particle lattice for deletion")
        ...
        for origin, destination, ribo_counts, origin_site_type, dest_site_type in swaps:
            ...
            # Note: site_lattice is a view and may be read-only
            # The region swap happens by moving particles, not changing site types directly
            ...
            for ribo_sp, count in ribo_counts.items():
                ...
                if particles is not None:
                    ...
                    for _ in range(count):
                        deleteParticle(particles, ox, oy, oz, pid)
                else:
                    # Fallback if we can't delete - this may not work properly
                    print(f"Warning: Cannot delete particles at ({ox},{oy},{oz})")
