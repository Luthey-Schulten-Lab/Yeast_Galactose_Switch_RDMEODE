...
v1.17:
- support differentitate ribosmes in ER, cec, tub and pma
- optimize the diffusion logic for R2 and G2. 
'''
...
enable_eff_G2_ribosome = True
IF_DGX = False
version = "1.17_combined"
...
parser.add_argument('--enable-chromosome', action='store_true',default=False, help='Enable chromosome regions and related functionality')
parser.add_argument('--enable-er', action='store_true',default=False, help='Enable ER regions and related functionality')
parser.add_argument('-ernum', '--er_num', type=int, default=4, help='Number of ER tunnels')
parser.add_argument('--enable-effective-ribosome', action='store_true',default=False, help='Enable effective ribosome case (includes both ribosome and ribosome_dummy)')
parser.add_argument('--enable_rna_tracking', action='store_true', default=False, help='Enable RNA tracking functionality')
...
print(f"RNA tracking support: {enable_rna_tracking}")
import datetime
...
membrane = np.load(dir_dgx + "plasmMembrane_connected.npy").astype(bool)
cellWall = boolLattice("cellWall") & ~ membrane
...
if enable_er:
    ...
    if enable_effective_ribosome:
        ...
        effective_cytoRibosomes = np.load(dir_dgx + "effective_cyto_ribosomes_ER_Marie_4152.npy").astype(bool)
        dummy_cytoRibosomoe = np.load(dir_dgx + "dummy_cyto_ribosomes_ER_Marie_4152.npy").astype(bool)
        # effective_erRibosomes = np.load(dir_dgx + "effective_er_ribosomes_ER_Marie_2151.npy").astype(bool)
        # dummy_erRibosomes = np.load(dir_dgx + "dummy_er_ribosomes_ER_Marie_2151.npy").astype(bool)
        effective_pmaRibosomes = np.load(dir_dgx + "effective_pma_ribosomes_ER_Marie_4256.npy").astype(bool)
        dummy_pmaRibosomes = np.load(dir_dgx + "dummy_pma_ribosomes_ER_Marie_4256.npy").astype(bool)
        effective_cecRibosomes = np.load(dir_dgx + "effective_cec_ribosomes_ER_Marie_4256.npy").astype(bool)
        dummy_cecRibosomes = np.load(dir_dgx + "dummy_cec_ribosomes_ER_Marie_4256.npy").astype(bool)
        effective_tubRibosomes = np.load(dir_dgx + "effective_tub_ribosomes_ER_Marie_4256.npy").astype(bool)
        dummy_tubRibosomes = np.load(dir_dgx + "dummy_tub_ribosomes_ER_Marie_4256.npy").astype(bool)
    ...
    pmaER = np.load(os.path.join(dir_dgx, ER_geo_dir, "pmER_fixed_geometry.npy")).astype(bool)
    tub_dir = "combined_tubes"
    if er_num == 4:
        tubER = np.load(os.path.join(dir_dgx, ER_geo_dir, "tubER_fixed_geometry.npy")).astype(bool)
        print("tubER is loaded from tubER_fixed_geometry.npy")
   
    elif er_num == 3:
        tubER = np.load(os.path.join(dir_dgx, tub_dir, "combined_tubes_3.npy")).astype(bool) | np.load(os.path.join(dir_dgx, tub_dir, "combined_tubes_2.npy")).astype(bool) | np.load(os.path.join(dir_dgx, tub_dir, "combined_tubes_1.npy")).astype(bool)
        print("tubER is loaded from combined_tubes_3.npy, combined_tubes_2.npy, combined_tubes_1.npy")
    elif er_num == 2:
        tubER = np.load(os.path.join(dir_dgx, tub_dir, "combined_tubes_2.npy")).astype(bool) | np.load(os.path.join(dir_dgx, tub_dir, "combined_tubes_1.npy")).astype(bool)
        print("tubER is loaded from combined_tubes_2.npy, combined_tubes_1.npy")
    elif er_num == 1:
        tubER = np.load(os.path.join(dir_dgx, tub_dir, "combined_tubes_1.npy")).astype(bool)
        print("tubER is loaded from combined_tubes_1.npy")
    else: 
        print("ER tunnels number should be 1, 2, 3 or 4, you have entered: ", er_num)
    endoplasmicReticulum = np.load(os.path.join(dir_dgx, ER_geo_dir, "cecER_fixed_geometry.npy")).astype(bool) | tubER
    ...
    elif enable_eff_G2_ribosome:
        print('Loading effective G2 ribosome regions for no ER case')
        # ribosome = np.load(dir_dgx + "effG2_ribosomes/effG2_Normribosomes_noER.npy").astype(bool)
        # ribosomeG2 = np.load(dir_dgx + "effG2_ribosomes/effG2_G2ribosomes_noER.npy").astype(bool)
        latticeData = pickle.load(lzma.open(dir_dgx + "lattice_ribosomes_ER_345964_isolated.pkl.xz", "rb"))
        siteMapER = {n: i for i, n in enumerate(latticeData['names'])}
        def boolLatticeER(x):
            return latticeData['lattice'] == siteMapER[x]
        ribosome = boolLatticeER("cytoRibosomes")
        ribosomeG2 = boolLatticeER("pmaRibosomes") | boolLatticeER("cecRibosomes")| boolLatticeER("tubRibosomes")
    else:
...
if enable_er:
    ...
    if enable_effective_ribosome:
        regions_to_compose.extend([
            ...
            (sim.region('dum_erRibosomes'), dummy_cytoRibosomoe),
            (sim.region('pmaRibosomes'), effective_pmaRibosomes),
            (sim.region('dum_pmaRibosomes'), dummy_pmaRibosomes),
            (sim.region('cecRibosomes'), effective_cecRibosomes),
            (sim.region('dum_cecRibosomes'), dummy_cecRibosomes),
            (sim.region('tubRibosomes'), effective_tubRibosomes),
            (sim.region('dum_tubRibosomes'), dummy_tubRibosomes),
    ...
    if enable_eff_G2_ribosome:
        regions_to_compose.append((sim.region('ribosomes'), ribosome))
        regions_to_compose.append((sim.region('ribosomesG2'), ribosomeG2))
    else:
...
if enable_er:
    ...
    ribosome_regions = [reg.ribosomesG2, reg.ribosomes]
else:
...
if checkpoint_file == "":
    ...
    if enable_er:
        ...
        for x, y, z in np.argwhere(sim.siteLattice == reg.ribosomes.idx):
            sp.ribosome.placeParticle(x, y, z, 1)
        for x, y, z in np.argwhere(sim.siteLattice == reg.ribosomesG2.idx):
            sp.ribosome.placeParticle(x, y, z, 1)
        print("ribosomes number:", np.sum(sim.siteLattice == reg.ribosomes.idx) + np.sum(sim.siteLattice == reg.ribosomesG2.idx))
        print(f"ribosomes: {np.sum(sim.siteLattice == reg.ribosomes.idx)}, ribosomesG2: {np.sum(sim.siteLattice == reg.ribosomesG2.idx)}")
    else:
        for x, y, z in np.argwhere(sim.siteLattice == reg.ribosomes.idx):
            sp.ribosome.placeParticle(x, y, z, 1)
...
if enable_er:
    ...
    for mrna in sim.speciesList.matchRegex("R.*"):
        if mrna.name != "R2":
            ...
            # [CHECKPOINT RESTART FAIL-SAFE] Escape routes from ER ribosome regions
            # These allow non-R2 mRNAs to escape if they end up in ER ribosome regions after checkpoint restart
            # Entry is blocked (diffusionZero by default), so normal simulation behavior is preserved
            sim.transitionRate(mrna, reg.pmaRibosomes, reg.cytoplasm, dc.mrna)
            sim.transitionRate(mrna, reg.cecRibosomes, reg.cytoplasm, dc.mrna)
            sim.transitionRate(mrna, reg.tubRibosomes, reg.cytoplasm, dc.mrna)
    ...
    # [CHECKPOINT RESTART FAIL-SAFE] R2 escape route from cytoRibosomes
    # Allows R2 to escape if it ends up in cytoRibosomes after checkpoint restart
    # Entry is blocked (diffusionZero by default), so normal simulation behavior is preserved
    sim.transitionRate(sp.R2, reg.cytoRibosomes, reg.cytoplasm, dc.mrna)
elif enable_eff_G2_ribosome:
    for mrna in sim.speciesList.matchRegex("R.*"):
        if mrna.name != "R2":
            sim.transitionRate(mrna, reg.nucleoplasm, reg.cytoplasm, dc.mrna)
            ...
            sim.transitionRate(mrna, reg.nucleoplasm, reg.nucleoplasm, dc.mrna)
            sim.transitionRate(mrna, reg.cytoplasm, reg.cytoplasm, dc.mrna)
            sim.transitionRate(mrna, reg.ribosomesG2, reg.ribosomesG2, dc.mrna)
            sim.transitionRate(mrna, reg.ribosomesG2, reg.cytoplasm, dc.mrna)
            sim.transitionRate(mrna, reg.cytoplasm, reg.ribosomesG2, dc.mrna)
else:
...
if enable_rna_tracking:
    if enable_er:
        # Special handling for ER version - non-R2 trackers
        for mrna in sim.speciesList.matchRegex("tracker_R.*"):
            if mrna.name != "tracker_R2":
                sim.transitionRate(mrna, reg.nucleoplasm, reg.cytoplasm, dc.mrna)
                sim.transitionRate(mrna, reg.cytoplasm, reg.nucleoplasm, sim.diffusionZero)
                sim.transitionRate(mrna, reg.nucleoplasm, reg.nucleoplasm, dc.mrna)
                sim.transitionRate(mrna, reg.cytoplasm, reg.cytoplasm, dc.mrna)
                ...
                sim.transitionRate(mrna, reg.cytoRibosomes, reg.cytoplasm, dc.mrna)
                sim.transitionRate(mrna, reg.cytoplasm, reg.cytoRibosomes, dc.mrna)
                # [CHECKPOINT RESTART FAIL-SAFE] Escape routes from ER ribosome regions for trackers
                sim.transitionRate(mrna, reg.pmaRibosomes, reg.cytoplasm, dc.mrna)
                sim.transitionRate(mrna, reg.cecRibosomes, reg.cytoplasm, dc.mrna)
                sim.transitionRate(mrna, reg.tubRibosomes, reg.cytoplasm, dc.mrna)

        # tracker_R2 special handling for ER
        sim.transitionRate(sp.tracker_R2, reg.nucleoplasm, reg.cytoplasm, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.cytoplasm, reg.nucleoplasm, sim.diffusionZero)
        sim.transitionRate(sp.tracker_R2, reg.cytoplasm, reg.cytoplasm, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.nucleoplasm, reg.nucleoplasm, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.cytoplasm, reg.pmaRibosomes, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.pmaRibosomes, reg.cytoplasm, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.pmaRibosomes, reg.pmaRibosomes, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.cytoplasm, reg.cecRibosomes, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.cecRibosomes, reg.cytoplasm, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.cecRibosomes, reg.cecRibosomes, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.cytoplasm, reg.tubRibosomes, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.tubRibosomes, reg.cytoplasm, dc.mrna)
        sim.transitionRate(sp.tracker_R2, reg.tubRibosomes, reg.tubRibosomes, dc.mrna)
        # [CHECKPOINT RESTART FAIL-SAFE] tracker_R2 escape route from cytoRibosomes
        sim.transitionRate(sp.tracker_R2, reg.cytoRibosomes, reg.cytoplasm, dc.mrna)
    else:
        # Standard mRNA diffusion
        for mrna in sim.speciesList.matchRegex("tracker_R.*"):
            sim.transitionRate(mrna, reg.nucleoplasm, reg.cytoplasm, dc.mrna)
...
if enable_er:
    ...
    for sps in [sp.G1, sp.G3, sp.G3i, sp.G4, sp.G4d, sp.G80, sp.G80d, sp.G80d_G3i, sp.Grep]:
        ...
        # [CHECKPOINT RESTART FAIL-SAFE] Escape routes from ER ribosome regions for proteins
        # Allows proteins to escape if they end up in ER ribosome regions after checkpoint restart
        sim.transitionRate(sps, reg.pmaRibosomes, reg.cytoplasm, sim.diffusionFast)
        sim.transitionRate(sps, reg.cecRibosomes, reg.cytoplasm, sim.diffusionFast)
        sim.transitionRate(sps, reg.tubRibosomes, reg.cytoplasm, sim.diffusionFast)
    ...
    sim.transitionRate(sp.G2, reg.pmaRibosomes, reg.pmaER, sim.diffusionFast)
    ...
    sim.transitionRate(sp.G2, reg.pmaER, reg.pmaRibosomes, sim.diffusionZero)
    ...
    sim.transitionRate(sp.G2, reg.pmaER, reg.cytoplasm, dc.prot)
    ...
    for sps in [sp.G1, sp.G2, sp.G3, sp.G3i, sp.G4, sp.G4d, sp.G80, sp.G80d, sp.G80d_G3i, sp.Grep]:
        ...
        sim.transitionRate(sps, reg.ribosomesG2, reg.cytoplasm, sim.diffusionFast)
        sim.transitionRate(sps, reg.cytoplasm, reg.ribosomesG2, sim.diffusionZero)
else:
...
for sps in [sp.G4, sp.G4d, sp.G80, sp.G80d]:
    if sps == sp.G80d:
        sim.transitionRate(sps, reg.nucleoplasm, reg.nucleoplasm, sim.diffusionFast)
    else:
...
sim.transitionRate(sp.G2, reg.cytoplasm, reg.plasmaMembrane, sim.diffusionFast)
...
# [CHECKPOINT RESTART FAIL-SAFE] Escape routes for dummy regions (effective ribosome + ER case)
# When restarting from checkpoint, particles may end up in dummy ribosome regions due to 
# slight geometry differences. Without escape routes, particles would be trapped and accumulate,
# causing particle overflow errors. These rules allow trapped particles to escape to cytoplasm.
if enable_er and enable_effective_ribosome:
    dummy_regions = [reg.dum_erRibosomes, reg.dum_pmaRibosomes, reg.dum_cecRibosomes, reg.dum_tubRibosomes]
    for dum_reg in dummy_regions:
        # mRNAs can escape from dummy regions
        for mrna in sim.speciesList.matchRegex("R.*"):
            sim.transitionRate(mrna, dum_reg, reg.cytoplasm, dc.mrna)
        # Proteins can escape from dummy regions
        for sps in [sp.G1, sp.G2, sp.G3, sp.G3i, sp.G4, sp.G4d, sp.G80, sp.G80d, sp.G80d_G3i, sp.Grep]:
            sim.transitionRate(sps, dum_reg, reg.cytoplasm, sim.diffusionFast)
